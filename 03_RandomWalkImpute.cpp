...
//     IntegerVector seq = as<IntegerVector>(sequences[i]); // Convert the list element to IntegerVector
//     if (seq.size() > 0 && !IntegerVector::is_na(seq[0])) {
...
List ImputeOneSequence(IntegerVector x, NumericVector initial_prob, NumericMatrix transition_matrix, int num_sim) {
  ...
  for (int i = 0; i < missing_groups.size(); i++) {
    ...
    List impute_result = GroupRandomWalk(x, group, initial_prob,  transition_matrix, num_sim);
...
// [[Rcpp::export]]
List SeqJointImpute(IntegerVector x, NumericVector initial_prob, NumericMatrix transition_matrix, int num_sim) {
  List group_imputes = ImputeOneSequence(x, initial_prob, transition_matrix, num_sim);
...
// [[Rcpp::export]]
List AllSeqJointImputeRCPP(List seq_list, NumericVector initial_prob, NumericMatrix transition_matrix, int num_sim) {
  int n = seq_list.size();
  List results(n);
...
AllSeqJointImputeR <- function(seq_list, initial_prob, transition_matrix, num_sim) {
